...
loadH5Data <- function(filename, genome=NULL)
{
  ...
  data <- sparseMatrix(i=dset$indices + 1,p=dset$indptr,
...
initVATEntity <- function(data.raw, title = "VAT", min.genes = 0, min.cells = 0,
                          ...
                          cell.scale = FALSE, scale.factor=10000,
                          verbose = TRUE){
  if(verbose) logging("create VAT Entity...")
  entity <- new(Class="VATEntity", data.raw=data.raw, title=title)
  ...
  umi.nums <- colSums(entity@data.raw) #apply(entity@data.raw, 2, function(x){return (sum(x))}) # expressed gene's UMI for each cell
  ...
  if(cell.scale){
    ...
    if(verbose){
      entity@data <- pblapply(index, function(x){return(entity@data[,x] * scale.factor /entity@cell.props$umi.nums[x])})
    }else{ 
      entity@data <- lapply(index, function(x){return(entity@data[,x] * scale.factor /entity@cell.props$umi.nums[x])})
    }
